...
class App extends Component {
  ...
  randomIndex = () => {
    ...
    while (imagesList[random].id === activeImage) {
      random = Math.floor(Math.random() * imagesList.length)
    }
  ...
  onClickedImage = id => {
    ...
    if (id === activeImage) {
      this.setState(prevState => ({
        ...
        activeImage: imagesList[this.randomIndex()].id,
      }))
    }
  }

  render() {
    const {score, timer, activeTabId, activeImage} = this.state
